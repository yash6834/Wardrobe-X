...
const Cart = () => {
  ...
  // 🔥 PULL EVERYTHING FROM YOUR GLOBAL CONTEXT
  const { cartItems, products, updateQuantity, removeFromCart } = useContext(ShopContext);
  ...
  useEffect(() => {
    ...
    for (const productId in cartItems) {
      for (const size in cartItems[productId]) {
        ...
        if (qty > 0) {
          // Find the actual product details from context
          const productData = products.find((p) => p._id === productId);
          
          // 🔥 OFFLINE FIX: If product data is missing, use a fallback placeholder!
          list.push({
            product: productData || {
              _id: productId,
              name: "Item (Offline - Pending Sync)",
              price: 0, 
              image: [], // Handled below in the UI
            },
            size: size,
            qty: qty,
          });
    ...
    setCartList(list);
  }, [cartItems, products]); // Re-run whenever global cart or products change
  ...
  const convertPrices = async () => {
    ...
    for (let item of cartList) {
      if (!navigator.onLine || item.product.price === 0) {
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
          ...
          <div className="flex-1 w-full space-y-8">
            {cartList.map(({ product, size, qty }) => {
              const price = convertedPrices[product._id] || product.price;

              return (
                <div key={`${product._id}-${size}`} className="flex gap-6 pb-8 border-b border-gray-100">
                  <div className="w-24 h-32 md:w-32 md:h-40 bg-[#F6F6F6] flex items-center justify-center overflow-hidden">
                    <img
                      src={
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className={`text-sm md:text-base font-medium uppercase ${product.price === 0 ? 'text-gray-400' : ''}`}>
                        {product.name}
                      </h3>
                      <p className="text-sm font-semibold">
                        {price > 0 ? (
                          ...
                          <span className="text-[10px] text-gray-400 font-normal uppercase">Calculating online...</span>
                        )}
                      </p>
                    </div>
          ...
          <div className="w-full lg:w-[380px] bg-[#FAFAFA] p-8">
            <h3 className="text-xs font-black uppercase mb-8">Order Summary</h3>
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span>
                  {shippingFee === 0
                    ? "FREE"
                    : `${currencySymbols[currency]}${shippingFee}`}
                </span>
              </div>
